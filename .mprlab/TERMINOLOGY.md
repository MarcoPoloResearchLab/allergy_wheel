...
- `CDN`: A content delivery network for external browser resources.
- `composition root`: The module that connects application components.
- `download page`: The website page that provides access to game installation.
- `ES module`: A JavaScript source file with explicit imports and exports.
- `game round`: One wheel spin and its result.
- `Google Analytics`: The external measurement service loaded by the current page.
- `LoopAware`: The external widget service loaded by the current page.
- `mobile package`: The game files and platform files for installation on a mobile device.
- `Node tooling`: Development tools that require the Node.js runtime.
- `offline play`: Game operation without a network connection.
- `parental gate`: An application control that restricts a specified action to an adult.
- `PWA`: A progressive web application that a browser can install.
- `service worker`: A browser script that can control network requests and cached resources.
