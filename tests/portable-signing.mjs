...
import { withAppleSigning } from '../scripts/apple-signing.mjs';
...
try {
    for (const scenario of ['success', 'build failure', 'interruption', 'import failure', 'cleanup failure', 'wrong app', 'expired profile', 'missing certificate', 'outside repository', 'wrong identity']) {
        ...
        if (scenario === 'missing certificate') await rm(environment.ALLERGY_WHEEL_APPLE_CERTIFICATE_PATH);
        if (scenario === 'outside repository') {
            const outside = join(root, 'outside.p12');
            await writeFile(outside, 'outside secret');
            environment.ALLERGY_WHEEL_APPLE_CERTIFICATE_PATH = outside;
        }
        const profile = {
            ...
            Entitlements: { 'application-identifier': `ABCDEFGHIJ.${scenario === 'wrong app' ? 'wrong.application' : 'com.mprlab.allergywheel'}`, 'get-task-allow': false },
        ...
        const build = async signing => {
            ...
            assert.equal(signing.environment.ALLERGY_WHEEL_APPLE_TEAM, 'ABCDEFGHIJ');
        ...
        let result;
        try {
            result = await withAppleSigning({ repositoryRoot, userDirectory, applicationIdentifier: 'com.mprlab.allergywheel', environment, execute, signal: controller.signal }, build);
        ...
        assert.equal(await readFile(environment.ALLERGY_WHEEL_APPLE_PROFILE_PATH, 'utf8'), 'signed profile fixture');
    }
