...
const KEYCHAIN_ENVIRONMENT = 'ALLERGY_WHEEL_APPLE_KEYCHAIN';
const CERTIFICATE_PATH = 'ALLERGY_WHEEL_APPLE_CERTIFICATE_PATH';
const CERTIFICATE_PASSWORD = 'ALLERGY_WHEEL_APPLE_CERTIFICATE_PASSWORD';
const PROFILE_PATH = 'ALLERGY_WHEEL_APPLE_PROFILE_PATH';

/** Execute a signing tool without putting passwords in process arguments or diagnostics. */
export async function executeSigningTool(name, args) {
    let input;
...
export async function withAppleSigning({ repositoryRoot, applicationIdentifier, environment = process.env, userDirectory = homedir(), execute = executeSigningTool, signal }, build) {
    ...
    const temporary = await realpath(await mkdtemp(join(tmpdir(), 'allergy-signing-')));
    ...
    const command = async (name, args) => {
        const response = await execute(name, args);
